...
{
    gArgs.AddArg("-addresstype", strprintf("What type of addresses to use (\"p2sh-segwit\", default: \"%s\")", FormatOutputType(DEFAULT_ADDRESS_TYPE)), ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
    ...
    gArgs.AddArg("-changetype", "What type of change to use (\"p2sh-segwit\"). Default is same as -addresstype, except when -addresstype=p2sh-segwit a native segwit output is used when sending to a native segwit address)", ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
    ...
    gArgs.AddArg("-dontchecktxfee", "Do not check tx fee before committing a tx", ArgsManager::ALLOW_BOOL, OptionsCategory::OPTIONS);
